...
/**
 * Two-phase steady-state solve: 1st order (reconstruct_const) for max_iter_1st steps,
 * then switch to 2nd order (recon_2nd) until L1 < 1e-5*R0 or max_iter total steps.
 * Speeds up convergence by getting close with robust 1st order, then refining with 2nd order.
 */
void solve_steady_2nd(const GriMesh& mesh, double* U, double gamma, const ProblemParams& params,
                      FluxFn flux_fn, ReconFn recon_2nd, double CFL = 0.1, int residual_stride = 50,
                      int max_iter = 100000, int max_iter_1st = 5000);
